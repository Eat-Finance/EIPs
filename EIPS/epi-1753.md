...
Returns the name of the token - e.g. `"My Permit"`.
...
Updates the contract with an ethereum address of who can issue permits.
...
Removes an ethereum address from the contract of who can issue permits.
...
Checks to see if an address has authority to issue or revoke permits.
...
Issues the permit for a given period of time.

``` js
function issue(address who, uint256 from, uint256 to)
```
...
Checks to see if the ethereum address has a valid permit.

``` js
function hasValid(address who)
```

#### purchase

Allows users to self procure a permit.

``` js
function purchase(uint256 from, uint256 to)
```
